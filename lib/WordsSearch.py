...
class WordsSearch():
    ...
    def Remark(self, text, replaceChar='*|*'):
        ts = self.FindAll(text)
        index = 0
        newts = []
        lts = None
        ...
        while (index < len(text)):
            step = 0
            af = 0
            for i in range(len(newts)):  # for (i = 0; i < ts.length; i++):
                f = newts[i]
                if f["Start"] + step == index:
                    af = 1
                    break
                elif f["End"] + step == index:
                    af = 2
                    break
                else:
                    af = 0
...
#     s = "中国|国人|zg人|乾清宫"
#     test = "我是中国人,你是国人吗，中国的人都是国人"
#     search = WordsSearch()
#     search.SetKeywords(s.split('|'))
#     txt = search.Remark(test)
#
#
#     print("-----------------------------------  WordsSearch  -----------------------------------" )
#
